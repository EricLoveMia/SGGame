	...
	* @throws
	...
	public static void changeMiddle(int type, int AttOrDef, General generalA, General generalD, Object virgin) {
		...
		if(type == 2){ // 如果是野战要注意把6个武将的技能都照顾到
			...
			}else if( (BF.getDefenceChief() != null && "13".equals(BF.getDefenceChief().getSkill())
					&& (new Random().nextInt(101) <= SkillFactory.getSkillByID(BF.getDefenceChief().getSkill()).getData()))
					|| (BF.getDefenceVice() != null && "13".equals(BF.getDefenceVice().getSkill())
					&& (new Random().nextInt(101) <= SkillFactory.getSkillByID(BF.getDefenceVice().getSkill()).getData()))
					|| (BF.getDefenceCounsellor() != null && "13".equals(BF.getDefenceCounsellor().getSkill())
					&& (new Random().nextInt(101) <= SkillFactory.getSkillByID(BF.getDefenceCounsellor().getSkill()).getData()))){ //神算 触发几率
				// 只有进攻方的技能触发
