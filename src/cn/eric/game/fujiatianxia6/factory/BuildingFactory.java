...
public class BuildingFactory {
    ...
    public static Map<Integer, List<String>> resetSkillMap = new HashMap<>();

    public static void initBuildings() {
        buildings = new ArrayList<>();
        buildings.add(new Building(1, "城墙", "防守时增加进攻方损失15%，每升一级增加5%的损失，仅守城适用", 15, 1, 5000, 5000));
        buildings.add(new Building(2, "弩炮", "防守时增加进攻方损失15%，每升一级增加5%的损失,守城野战适用", 15, 1, 5000, 5000));
        ...
        buildings.add(new Building(5, "武器阁", "购买武将专属的地方，每升一级降低购买价格10%", 50, 4, 5000, 5000));
        ...
        buildings.add(new Building(11, "重修堂", "武将重修技能的地方，花费一定的金额可以重修技能", 25, 4, 20000, 500000));

        resetSkillMap.put(50000, Arrays.asList("18", "22", "20", "26", "30"));
        resetSkillMap.put(30000, Arrays.asList("17", "21", "19", "46", "36", "40"));
        resetSkillMap.put(10000, Arrays.asList("44", "45", "56", "52", "51", "62"));
    }
            ...
            while (true) {
                ...
                if (choise == 0) {
                    ...
                    System.out.println("数字错误，请重新选择");
                        ...
                        || (choise == 7 && (countBuildsInCity(city, 7) < 1)) || (choise == 8 && (countBuildsInCity(city, 8) < 1))
                        || (choise == 9 && (countBuildsInCity(city, 9) < 1)) || (choise == 10 && (countBuildsInCity(city, 10) < 1))
                        || (choise == 11 && (countBuildsInCity(city, 11) < 1))) {
...
//		General general = GeneralFactory.getGeneralById(belongTo.toString());
        ...
        if (player.isReboot()) {
            if (collect.size() == 0) {
                ...
                while (iterator.hasNext()) {
                    ...
                    Building building = next.getValue();
                    if (building.level < defence.getType()) {
                        if (building.upgradeLevel < player.getMoney() / 3) {
                            // 等级不能高于城市等级
                            if (building.level < defence.getType()) {
                                choise = next.getKey();
                                break;
    ...
    public static void checkSpecialBuilding(City defence, General player) {
        List<Building> buildings = defence.getBildings();
        if (buildings.size() > 0) {
            for (Building building : buildings) {
                if (building.id == 5) {
                    // 购买武器
                ...
                if (building.id == 11) {
                    // 重修技能
                    System.out.println("欢迎来到技能重修阁，可以花费一定的金币给武将重修技能");
                    System.out.println("请选择一名武将来重修技能");
                    General general = GeneralFactory.chooseOneGeneral(player);
                    if (general != null) {
                        String skill = general.getSkill();
                        System.out.println("当前武将的技能是:" + Objects.requireNonNull(SkillFactory.getSkillByID(skill)).getName()
                                + "：" + Objects.requireNonNull(SkillFactory.getSkillByID(skill)).getMemo());

                        System.out.println("请选择重修技能的档次：0 取消, 当前持有金币" + player.getMoney());
                        int index = 1;
                        Map<Integer, Integer> indexChooseMap = new HashMap<>();
                        for (Map.Entry<Integer, List<String>> entry : resetSkillMap.entrySet()) {
                            indexChooseMap.put(index, entry.getKey());
                            System.out.println(index++ + "-金额" + entry.getKey() + ":可选择的技能有");
                            for (String skillId : entry.getValue()) {
                                System.out.println(SkillFactory.getSkillByID(skillId).getName() + "：" + SkillFactory.getSkillByID(skillId).getMemo());
                            }
                            System.out.println();
                        }
                        ...
                        if (choose > 0 && choose <= 3) {
                            Integer key = indexChooseMap.get(choose);
                            if (player.getMoney() > key) {
								List<String> list = resetSkillMap.get(key);
								SkillFactory.resetSkill(list,general,player,key);
                            } else {
